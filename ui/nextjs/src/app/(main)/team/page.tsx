    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      {selectedMember && (
        <Card className="glass border-indigo-400/30 overflow-hidden" id="member-detail"><CardContent className="p-5">
          ...
          <div className="flex gap-3 mt-5"><Generate1on1Button person={selectedMember.name} /><a href={`https://linear.app/maiolabs/search?q=${encodeURIComponent(selectedMember.linear)}`} target="_blank" rel="noopener" className="inline-flex items-center rounded-lg px-3 py-2 text-xs glass-hover border border-border/40">Ver Linear</a></div>
        </CardContent></Card>
      )}
    </div>
  )
